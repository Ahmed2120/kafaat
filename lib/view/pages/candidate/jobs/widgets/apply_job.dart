...
class ApplyJobPage extends StatefulWidget {
  const ApplyJobPage({Key? key, required this.slugId}) : super(key: key);
  final String slugId;
    ...
    return Scaffold(
      body: Column(
        children: [
          const MainAppBar(),
          Expanded(
            child: (jobs.jobDetailsStatus == DataStatus.successed &&
                    jobs.jobDetailsModel != null)
                ? Builder(builder: (context) {
                    String jobDes = convertHtmlToPlainText(
                            jobs.jobDetailsModel?.jobDetail?.content! ?? '')
                        .replaceAll(RegExp(r'\n\n'), '\n');
                    return Padding(
                                      ...
                                      Expanded(
                                        ...
                                        child: Chip(
                                          label: FittedBox(
                                            child: MainText(
                                              text:
                                                  '${jobs.jobDetailsModel?.jobDetail?.salaryMin ?? ''} - ${jobs.jobDetailsModel?.jobDetail?.salaryMax}',
                                              font: 14,
                                  ...
                                  readMore
                                      ? Column(
                                          ...
                                          children: [
                                            ...
                                            const SizedBox(height: 8),
                                            if (jobs.jobDetailsModel?.jobDetail
                                                    ?.experience !=
                                                null)
                                              MainText(
                                                text:
                                                    '${jobs.jobDetailsModel?.jobDetail?.experience?.toString() ?? ''} Years',
                                                font: 13,
                                                weight: FontWeight.w400,
                                              ),
                                            const SizedBox(height: 12),
                ...
                : jobs.jobDetailsStatus == DataStatus.error
                    ? RetryWidget(retryCallback: () {
                        jobs.jobDetails(slugId: widget.slugId);
                      })
                    : const SizedBox(),
          ),
        ],
